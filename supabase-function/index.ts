// دالة مجيب السحابية — تنشر في Supabase Edge Functions باسم: mujeeb-chat
...
Deno.serve(async (req: Request) => {
  ...
  try {
    const { question, history = [], user_email = null } = await req.json();
    ...
    const system = `أنت "${S.assistant_name ?? "مجيب"}"، المساعد الذكي الرسمي لموظفي هيئة المشاريع والمناقصات والمحتوى المحلي في سلطنة عُمان.
قواعد صارمة:
